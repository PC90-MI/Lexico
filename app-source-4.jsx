...
// ─── fetchVerbBatch ────────────────────────────────────────────────────────────
async function fetchVerbBatch(type) {
  const prompt = type==="translate"
    ? `Generate a batch of 7 Italian sentences for an Italian learner to translate into English, at CEFR B2 level. Each sentence should be built around a verb in past, present, or future tense — mix tenses across the batch (do not use the same tense for every item) and vary subjects/persons naturally.\n`+
      `For each item give "it": the Italian sentence, and "en": one natural, idiomatic reference English translation.\n`+
    ...
    : `Generate a batch of 7 English verb-tense practice exercises for an Italian learner at CEFR B2 level. Mix past, present, and future tenses across the batch (do not use the same tense for every item) and vary subjects/persons naturally.\n`+
      `Each item: a natural English sentence with exactly ONE verb blanked out (write the blank as "___" inside the sentence), testing the correct conjugated form for that tense/context. Also give "infinitive": the bare infinitive of that verb without "to". Give "answer": the single correct conjugated form (just the verb, not the whole sentence).\n`+
      `Reply ONLY with a valid JSON array, no markdown, no commentary. Format:\n`+
...
const BUILD_LABEL = "17 set 2026 · 02:19";
...
export default function VocabApp() {
  ...
  const [verbExerciseType, setVerbExerciseType] = useState("blank"); // "blank" | "translate"
  ...
  // ── verbi helpers ──
  async function startVerbSession(type) {
    const t = type || verbExerciseType;
    if (!getApiKey()) { setVerbError("noKey"); return; }
    setVerbLoading(true); setVerbError(null);
    try {
      const batch = await fetchVerbBatch(t);
  ...
  function switchVerbType(newType) {
    if (newType===verbExerciseType || verbLoading) return;
    setVerbExerciseType(newType);
    setVerbQueue([]); setVerbCurrent(null); setVerbAnswer(""); setVerbFeedback(null); setVerbError(null);
    startVerbSession(newType);
  }
  ...
  async function nextVerbCard() {
    ...
    setVerbLoading(true); setVerbError(null);
    try {
      const batch = await fetchVerbBatch(verbExerciseType);
      setVerbCurrent(batch[0]); setVerbQueue(batch.slice(1));
        ...
        {mode==="verbi"&&(
          <div>
            <div style={{display:"flex",gap:8,marginBottom:16}}>
              <button onClick={()=>switchVerbType("blank")} style={{flex:1,padding:"10px 12px",borderRadius:10,fontSize:13,fontWeight:600,
                border:verbExerciseType==="blank"?"1px solid #B5562B":"1px solid #DDD3C2",
                background:verbExerciseType==="blank"?"#B5562B":"#fff",
                color:verbExerciseType==="blank"?"#fff":"#8A7D68"}}>
                Completamento
              </button>
              <button onClick={()=>switchVerbType("translate")} style={{flex:1,padding:"10px 12px",borderRadius:10,fontSize:13,fontWeight:600,
                border:verbExerciseType==="translate"?"1px solid #B5562B":"1px solid #DDD3C2",
                background:verbExerciseType==="translate"?"#B5562B":"#fff",
                color:verbExerciseType==="translate"?"#fff":"#8A7D68"}}>
                Traduzione
              </button>
            </div>
            {verbError==="noKey" ? (
              ...
              <>
                <div style={{fontSize:13,color:"#8A7D68",marginBottom:10}}>
                  Punteggio: {verbStats.correct}/{verbStats.total}
                </div>
                <div style={{background:"#fff",border:"1px solid #E7DFD3",borderRadius:14,padding:"32px 24px",textAlign:"center"}}>
                  {verbCurrent.type==="blank" ? (() => {
                    const parts = verbCurrent.sentence.split("___");
                    const before = parts[0] ?? "";
                    const after  = parts.slice(1).join("___") ?? "";
                    const blankBorder = verbFeedback==="correct"?"2px solid #4A7A4A":verbFeedback==="wrong"?"2px solid #B5562B":"2px solid #DDD3C2";
                    const blankBg     = verbFeedback==="correct"?"#F1F7EF":verbFeedback==="wrong"?"#FBF0EC":"#fff";
                    return (
                      <>
                        <div style={{fontSize:12,letterSpacing:1.5,color:"#B5562B",fontWeight:700,marginBottom:18}}>COMPLETA LA FRASE</div>
                        <div className="display" style={{fontSize:21,fontWeight:600,lineHeight:1.7,marginBottom:22,
                          display:"flex",flexWrap:"wrap",alignItems:"baseline",justifyContent:"center",gap:"0 8px"}}>
                          <span>{before}</span>
                          <input ref={verbInputRef} value={verbAnswer} onChange={e=>setVerbAnswer(e.target.value)} onKeyDown={handleVerbKey}
                            disabled={verbFeedback!==null} autoFocus placeholder="…"
                            style={{width:Math.max(90,verbAnswer.length*13+40)+"px",padding:"4px 10px",borderRadius:6,fontSize:20,
                              textAlign:"center",fontFamily:"inherit",fontWeight:600,border:blankBorder,background:blankBg}}/>
                          <span style={{fontSize:13,color:"#9C8F7A",fontStyle:"italic",fontWeight:400}}>({verbCurrent.infinitive})</span>
                          <span>{after}</span>
                        </div>
                        {verbFeedback==="wrong"&&(
                          <div style={{marginBottom:18,fontSize:14,color:"#B5562B"}}>Risposta corretta: <strong>{verbCurrent.answer}</strong></div>
                        )}
                        {verbFeedback===null
                    ...
                    <>
                      ...
                      <div className="display" style={{fontSize:21,fontWeight:600,marginBottom:18,lineHeight:1.45}}>{verbCurrent.it}</div>
                      {verbFeedback!=="revealed" ? (
                        <>
                          <textarea ref={verbInputRef} value={verbAnswer} onChange={e=>setVerbAnswer(e.target.value)}
                            onKeyDown={e=>{ if(e.key==="Enter"&&!e.shiftKey){ e.preventDefault(); revealTranslation(); } }}
                            autoFocus placeholder="Scrivi qui la tua traduzione in inglese…" rows={2}
                            style={{width:"100%",maxWidth:420,padding:"12px 14px",borderRadius:8,fontSize:16,textAlign:"center",
                              border:"1px solid #DDD3C2",background:"#fff",fontFamily:"inherit",resize:"none",marginBottom:16,display:"block",marginLeft:"auto",marginRight:"auto"}}/>
                          <ActionButton onClick={revealTranslation} color="#B5562B">Mostra traduzione</ActionButton>
                        </>
                      ) : (
                        <>
                          {verbAnswer.trim() && (
                            <div style={{marginBottom:12,fontSize:13,color:"#8A7D68"}}>
                              La tua traduzione: <span style={{color:"#2B2521"}}>{verbAnswer}</span>
                            </div>
                          )}
